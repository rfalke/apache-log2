...
function log(message) {
    if (apache_log.data.settings().debug) {
        console.log("[" + apache_log.data.settings().appname + "] " + message);
    }
}
...
apache_log.logger = function (request, response) {
    ...
    response.end = function (data) {
        ...
        responseEnd(request, responseBodyLen, responseStatusCode); // Call the logging function with internal response scoping varables
    };
